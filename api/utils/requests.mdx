...
<ParamField path="market" type="string" required>
  Market symbol (e.g., "BTC_USDT", "ETH_USDT")
</ParamField>
...
const depth = await getDepth("BTC_USDT");
...
const trades = await getTrades("BTC_USDT");
...
const klines = await getKlines("BTC_USDT", "1h", oneDayAgo);
...
<ParamField path="order" type="CreateOrder" required>
  Order parameters:
  - `market` (string): Market symbol (e.g., "BTC_USDT")
...
async function initializeTrading() {
  ...
  const klines = await getKlines("BTC_USDT", "1h", startTime);
